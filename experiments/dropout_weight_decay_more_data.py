...
def get_grid_param_list():
    ...
    default_params = [
        {
            "save_folder": "/data/schirrmr/schirrmr/reversible/experiments/dropout-weight-decay-more-data",
    ...
    ival_params = [{"start_ms": 500, "stop_ms": 3000}]
    training_params = dictlistprod({"max_epochs": [300]})
...
def run_exp(
    ...
    import torch as th
    def split_into_parts(inputs):
        all_ins = []
        for i_start in [0, 128, 256, 384]:
            ins = inputs[:, :, i_start:i_start + 256]
            all_ins.append(ins)
        return th.cat(all_ins, dim=0)

    def split_both_into_parts(inputs_per_class):
        return [split_into_parts(i) for i in inputs_per_class]

    train_inputs = split_both_into_parts(train_inputs)
    test_inputs = split_both_into_parts(test_inputs)
            ...
            if i_epoch > 0:
                ...
                for i_class in range(2):
                    ...
                    log_probs = model_and_dist.get_total_log_prob(
                        i_class, class_ins + (th.rand_like(
                            class_ins) - 0.5) * noise_factor)
